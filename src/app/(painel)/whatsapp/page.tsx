...
import { montarAparelho, formatarTelefone, type UserDevice } from '@/lib/aparelho'
...
export default async function WhatsappPage({ searchParams }: { searchParams: Promise<{ f?: string; q?: string }> }) {
  ...
  const [{ data: users }, { data: opens }, { data: scanLinhas }, { data: scanRuns }, { data: contatos }, { data: devices }] = await Promise.all([
  ...
  const aparelhos = new Map(((devices ?? []) as UserDevice[]).map((d) => [d.user_id, montarAparelho(d, null, [])]))
  ...
  for (const al of ws.alunos) {
    ...
    perfis[p.id] = {
      ...
      celular: aparelhos.get(p.id)?.modelo ?? null,
      versao: aparelhos.get(p.id)?.versao ?? null,
      canalApp: aparelhos.get(p.id)?.canal ?? null,
    }
  }
